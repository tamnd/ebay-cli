...
// surface is a different URL (unlike a single GraphQL endpoint), so the URL is
// what distinguishes one response from another. A hit is served when the file is
// younger than ttl.
type cache struct {
	dir string
	ttl time.Duration
